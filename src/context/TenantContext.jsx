...
export function TenantProvider({ children }) {
  ...
  useEffect(() => {
    const savedSession = localStorage.getItem('meganntv_session')
    if (savedSession) {
      try {
  ...
  useEffect(() => {
    ...
    const handleActivity = () => {
      const currentSession = JSON.parse(localStorage.getItem('meganntv_session') || '{}')
      if (currentSession) {
        currentSession.lastActivity = Date.now()
        localStorage.setItem('meganntv_session', JSON.stringify(currentSession))
      }
    }
  ...
  const createSession = (user, organizationId) => {
    ...
    localStorage.setItem('meganntv_session', JSON.stringify(newSession))
    return newSession
  }
  ...
  const switchOrganization = (organizationId) => {
    ...
    setSession(updated)
    localStorage.setItem('meganntv_session', JSON.stringify(updated))
  }
